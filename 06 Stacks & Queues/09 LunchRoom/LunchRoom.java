...
public class LunchRoom
{
   ...
   public static void main(String[] args){
      PriorityQueue<Customer> dave = new PriorityQueue<Customer>();
      Queue<Customer> fre = new LinkedList<Customer>(); // for different classes outta the line
      Queue<Customer> sop = new LinkedList<Customer>();
      Queue<Customer> jun = new LinkedList<Customer>();
      Queue<Customer> sen = new LinkedList<Customer>();
      /*  write code for the simulation   */
      for(int a = 0; a <= TIME; a++){ // during the 18 hours
         if(!dave.isEmpty()){ //removes any finished customers and adds to line
            if(!(dave.peek().check(a))) //takes out if necessary
               switch(dave.peek().myClass()){
                  case 2020:
                     sen.add(dave.remove());
                     break;
                  case 2021:
         ...
         double zeroPerson = ((Math.pow(.2,0) * Math.exp(-.2)/1)); // lots of the probability stuff
         double onePerson = ((Math.pow(.2,1) * Math.exp(-.2)/1));
         double twoPerson = ((Math.pow(.2,2) * Math.exp(-.2)/2));
         double threePerson = ((Math.pow(.2,3) * Math.exp(-.2)/6));
         double random = Math.random();
         int added = 0;
         if(random < zeroPerson)
            added = 0;
         else if(random < (zeroPerson + onePerson))
            added = 1;
         else if(random < (zeroPerson + onePerson + twoPerson))
            added = 2;
         else if(random < (zeroPerson + onePerson + twoPerson+ threePerson))
            added = 3;
         else{ added = 4;
         }
         
         
         for(int s = 0; s <added; s++){ //adds customer
            Customer eek = new Customer(a);
            dave.add(eek);
         }
         ...
         display(a,dave);
         if(!dave.isEmpty()) //takes the order
            if(!dave.peek().checkOrderTaken())
               dave.peek().takeOrder(a);
      ...
      int b = TIME + 1; // repeats the process, except no new customers come in
      while(!dave.isEmpty()){ //
         if(!dave.isEmpty()){ //removes any finished customers and adds to line
            if(!(dave.peek().check(b)))
               switch(dave.peek().myClass()){ // removes if necessary and adds to proper queue for the end
                  case 2020:
                     sen.add(dave.remove());
                     break;
                  case 2021:
         ...
         display(b,dave); // displays
         if(!dave.isEmpty()) //takes the order
            if(!dave.peek().checkOrderTaken())
               dave.peek().takeOrder(b);
         b++;
      }
      ...
      /* report the data  */
      calculate("Senior",sen); // prints 
      calculate("Junior",jun); // prints
      calculate("Sophmore",sop); // prints 
      calculate("Freshman",fre); // prints
      
   }
   
   public static void calculate(String f,Queue q){
   int total = checkQueueLength(q); // gets total number
   double averageWaitTime = 0; // variable for average wait time
   int longestWaitTime = 0; // tracks longest wait time
   Iterator<Customer> d = q.iterator(); // iterator to go through
      while(d.hasNext()){ // goes through and adjusts variables
         int tempWaitTime = d.next().totalWaitTime();
         averageWaitTime += 1.00 * tempWaitTime;
         if(longestWaitTime < tempWaitTime)
            longestWaitTime = tempWaitTime;
      }
      averageWaitTime = averageWaitTime * 1.00 /total;// calculates average wait time
      System.out.println(f + "\t\t" + total + "\t\t" + longestWaitTime + "\t\t" + averageWaitTime); // prints everything
         
   }
   public static int checkQueueLength(Queue q){ // checks the length of a queue using an iterator
      int e = 0;
      Iterator<Customer> d = q.iterator();
      while(d.hasNext()){
         e++;
         d.next();
      }
   ...
   public static void display(int min, PriorityQueue<Customer> q)
   {
      Iterator<Customer> d = q.iterator(); // deletes customers that are done
      while(d.hasNext()){
         if(!(d.next().check(min)))
            d.remove();
      
      }
      String s = (min + ":  ["); // opening
      
      Iterator<Customer> e = q.iterator(); // adds customers
      while(e.hasNext()){
         s += e.next();
         if(e.hasNext())
            s+= ",";
      ...
      s += "]"; // closes
      System.out.println(s);   // returns string   }
   }
}
...
class Customer implements Comparable<Customer>
{
   ...
   public Customer(int arrivedTime/*, /*int servedNumber,/*int c,int gClass*/){ // initializing private data
      waitTime = (int)(Math.random() * 6) + 2; // random wait time
      arrivalTime = arrivedTime; 
      orderTaken = false;
      //numberServed = servedNumber;
      //enterLength = c;
      double e = Math.random(); // determines class
      if(e >.75)
         gradClass = 2020;
      else if(e >.50)
         gradClass = 2021;
   ...
   public boolean check(int e){ // checks if they should be reomved
      if(!orderTaken)
         return true;
      if(servedTime + waitTime <= e)
         return false;
      else 
         return true;
   }
   ...
   public boolean checkOrderTaken(){ // checks if order taken
      return orderTaken;
   }
   public void takeOrder(int a){ // changes order taken and takes the time they are served at 
      orderTaken = true;
      servedTime = a;
   }
   public int totalWaitTime(){ // returns total wait time
      return (servedTime + waitTime - arrivalTime);
   }
   
   public int myClass(){ // returns what class they in
      return gradClass;
   }
   public int myArrivalTime(){ // returns their arrival time
      return arrivalTime;
   }
   public int compareTo(Customer e){ // returns whether they should be in front by means of an int
      if(orderTaken)
         return -1;
      if((gradClass - e.myClass()) == 0)
         return arrivalTime - e.myArrivalTime();
      else 
         return gradClass - e.myClass();
   }
}
