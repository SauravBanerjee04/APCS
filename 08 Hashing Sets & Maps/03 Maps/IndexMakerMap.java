...
class DocumentIndex extends TreeMap<String, TreeSet<Integer>>
{
   ...
   */
   ...
   {
      String[] d = str.split("[., \"!?]"); // splits then calls add
      for(String e : d){
         addWord(e,lineNum);
      }
   ...
   */
   ...
   {
      if(word.equals("") || word.equals(" ")) // if empty space does nothing
         return;
      String d = word.toUpperCase(); // makes uppercase
      if(get(d) == null){ // if new
         TreeSet<Integer> e = new TreeSet<Integer>();
         e.add(new Integer(lineNum));
         put(d,e);
         
      }
      else{
         TreeSet<Integer> e = get(d); // otherwise adds to the TreeSet
         e.add(new Integer(lineNum));
      }
   ...
   {
      String e = ""; // new string
      for(String f: keySet()){ // goes through keys
         TreeSet<Integer> tree = get(f);
         String indices = ""; // goes through indices
         for(Integer l : tree)
            indices += (l.toString() + ", ");
